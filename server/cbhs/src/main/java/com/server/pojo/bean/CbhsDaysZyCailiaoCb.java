...
@DynamicInsert
@Entity
@Table(name = "CbhsDaysZyCailiaoCb")
@JsonIgnoreProperties({"hibernateLazyInitializer", "handler"})
@ApiModel(value="自营材料每日成本对象",description="自营材料每日成本对象")
public class CbhsDaysZyCailiaoCb implements Serializable {
	private static final long serialVersionUID = 1L;
	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	@ApiModelProperty(value = "主键（材料成本）(创建不填|修改必填)",required=false)
	private Integer oid= 0;// 主键（材料成本）(创建不填|修改必填)
	...
	@Column(name = "cailiaoKemuOid", length = 50, nullable = true, unique = false,columnDefinition="Integer default 0")
	@ApiModelProperty(value = "必填|对应的材料科目ID",required=false)
	private Integer cailiaoKemuOid= 0;// 必填|对应的材料科目ID
	...
	public Integer getCailiaoKemuOid() { 

    	return this.cailiaoKemuOid;

	}

	public void setCailiaoKemuOid(Integer cailiaoKemuOid) {

...
@DynamicInsert
@Entity
@Table(name = "CbhsSettleDown")
@JsonIgnoreProperties({"hibernateLazyInitializer", "handler"})
@ApiModel(value="对下结算对象",description="对下结算对象")
public class CbhsSettleDown implements Serializable {
	private static final long serialVersionUID = 1L;
	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	@ApiModelProperty(value = "主键(对上结算)(创建不填|修改必填)",required=false)
	private Integer oid= 0;// 主键(对上结算)(创建不填|修改必填)
	...
	@Column(name = "cbhsSettleUpOid", nullable = true, unique = false,columnDefinition="Integer default 0")
	@ApiModelProperty(value = "必填|对上结算的ID",required=false)
	private Integer cbhsSettleUpOid= 0;// 必填|对上结算的ID
	...
	@ApiModelProperty(value = "结算数量",required=false)
	private BigDecimal count= new BigDecimal(0);// 结算数量

...
/****
 * 对应编码表的excel文件，生成对应的编码表对象。
 * 
 * @author Administrator
 * 
 */
public class BmOut extends AbstractProcess {

	private BufferedWriter bw = null;

	private ArrayList<List<mySdjsBM>> tables = new ArrayList<List<mySdjsBM>>();

	private ArrayList<String> tableNames = new ArrayList<String>();

	private ArrayList<mySdjsBM> curTable = null;
	private HashMap<String, mySdjsBM> uiSdjsBms = new HashMap<String, mySdjsBM>();

	private static final String DIR = File.separator + "0.gen_code\\base_api\\src\\main\\java\\com\\yqt\\api\\static_";

	private static final String androidDir = File.separator + "3.android\\BaseFrame\\src\\main\\java\\com\\team\\baseframe\\model";

	public BmOut(String targetDir, boolean isAndroid) throws IOException {
		...
		if (isAndroid) {
			targetFile = targetDir + androidDir + File.separator + "YqtBmConstants.java";
		} else {
			targetFile = targetDir + DIR + File.separator + "YqtBmConstants.java";
		}
	...
	public void finish() throws Exception {
		...
		for (int i = 0; i < tables.size(); i++) {
			...
			if (i == 0) {
				...
				bw.newLine();
				bw.write("		List<YqtBm>  childs3=null;");
			} else {
			...
			List<mySdjsBM> list = tables.get(i);

			for (mySdjsBM kValue : list) {
	 ...
	 * @throws IOException
	 */
	private void addChildBm(mySdjsBM parent, int index) throws IOException {
		...
		bw.newLine();
		for (mySdjsBM child : parent.getChilds()) {
	...
	public Iterator<ILevelObj> processFile(final FileImpl ifile, int index) throws Exception {
		...
		if (index == 0) {
			...
			bw.newLine();
			bw.newLine();
			bw.write("	private static final HashMap<String, List<YqtBm>> bmTables = new HashMap();");
			bw.newLine();
	...
	private mySdjsBM curBmObj = null;
	...
	public Iterator<ILevelObj> processFiled(FieldObj field, int index) throws Exception {
		...
		curBmObj = new mySdjsBM(codes[pathCount - 1], field.toString());
	...
	class mySdjsBM {
		...
		private List<mySdjsBM> childs = null;

		public mySdjsBM(int bm, String name) {
			this.bm = bm;
			this.name = name;
			this.childs = new ArrayList<mySdjsBM>();
		}
		...
		public List<mySdjsBM> getChilds() {
			return this.childs;
		}

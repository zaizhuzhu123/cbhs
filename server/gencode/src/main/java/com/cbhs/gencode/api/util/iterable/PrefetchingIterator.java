package com.cbhs.gencode.api.util.iterable;
 ...
 * cache it with {@link #setPrefetchedNext(Object)}. The cached object is then
 * set to {@code null} in {@link #next()}. So you only have to implement one
 * method, {@code fetchNextOrNull} which returns {@code null} when the iteration
 * has reached the end, and you're done.
 */
public abstract class PrefetchingIterator<T> implements Iterator<T> {
	 ...
	 * fetch it again.
	 *
	 * @return {@code true} if there was a next item to return in the next call
	 *         to {@link #next()}.
	 */
	 ...
	 *         {@link NoSuchElementException} if there's no more items to
	 *         return.
	 */
	public T next() {
		if (!hasNext()) {

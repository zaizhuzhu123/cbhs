...
public class MenuFile implements ILevelObj {

	private File rootFile = null;

	public MenuFile(String path) throws Exception {
		path = path + "\\menu";
		...
		return new PrefetchingIterator<ILevelObj>() {
			...
			protected ILevelObj fetchNextOrNull() {
				while (index < files.length) {
					...
					if (files[index].getName().endsWith("Menu.xlsx")) {
						FileImpl excelFile = null;
						try {
							excelFile = new FileImpl(files[index], 2, 5);
